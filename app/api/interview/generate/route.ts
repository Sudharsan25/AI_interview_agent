...
export async function POST(request: NextRequest){
    ...
    const currentUserArray = await db.select()
                                     .from(user)
    ...
    const { role, level, type, techstack, length, jobDesc, companyDetails, specialization }= await request.json();

    try{
        if (!role || !level || !type || !techstack || !userId || !length || !jobDesc) {
    ...
    const { interviewId } = await db.transaction(async (tx) => {
        ...
        // STEP 1: Insert the main interview record using the 'tx' client.
        const newInterviewResult = await tx.insert(interviews)
            .values(interviewData)
            .returning({ insertedId: interviews.id });

        const id = newInterviewResult[0]?.insertedId;

        // A sanity check to ensure the insert worked. If not, rollback.
        if (!id) {
            throw new Error("Failed to create interview record.");
        }

        // STEP 2: Prepare the questions array using the new interview ID.
        const questionsToInsert = questionsArray.map(questionText => ({
            questionText: questionText.trim(),
            interviewId: id,
        }));

        // STEP 3: Insert all questions using the same 'tx' client.
        await tx.insert(questions).values(questionsToInsert);
        
        // STEP 4: Return the new ID from the transaction block.
